...
function RenderSurpriseSteps({
  ...
  useEffect(() => {
    async function fetchAndSetPexelsUrl() {
      try {
        if (preferences.destinationCity.trim() !== "") {
          const url = await getPexelsUrl(preferences.destinationCity);
          setPexelsUrl(url);
        }
      } catch (error) {
        console.error(error);
      ...
      >
        ...
        {currentStep === 6 && (
          <Step6
            ...
            next={next}
            arrowTopRight={arrowTopRight}
            NUM_OF_TRAVELERS_OPTIONS={NUM_OF_TRAVELERS_OPTIONS}
          />
        )}
        {currentStep === 7 && (
          <Step7
            ...
            next={next}
            arrowTopRight={arrowTopRight}
            AMBIENCE_OPTIONS={AMBIENCE_OPTIONS}
          />
        )}
      </motion.div>
    </AnimatePresence>
...
export default RenderSurpriseSteps;

RenderSurpriseSteps.propTypes = {
