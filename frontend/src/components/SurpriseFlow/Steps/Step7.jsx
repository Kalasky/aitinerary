...
const Step7 = ({
  ...
  const selectedOption = AMBIENCE_OPTIONS.find(
    (option) => option.value === preferences.ambience
  ) || { label: "Select trip ambience" };

  const handleAmbienceChange = (value) => {
    handleSelectChange("ambience", value);
  };
  ...
  return (
    <>
      <SimpleToast
        title="Please choose the trip ambience"
        description="What type of atmosphere do you want?"
        buttonText="Close"
            ...
            <div className="absolute top-full z-10 w-full bg-[#9c9c9c] bg-opacity-80 border-[1px] border-gray-400 rounded-[14px] px-4 py-4 text-base">
              {AMBIENCE_OPTIONS.map((option) => (
                <div
                  ...
                  onClick={() => handleAmbienceChange(option.value)}
                >
...
Step7.propTypes = {
  ...
  AMBIENCE_OPTIONS: PropTypes.array.isRequired,
